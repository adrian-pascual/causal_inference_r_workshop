...
standardized_model <- lm(
  wait_minutes_actual_avg ~ ns(wait_minutes_posted_avg, df = 5)*park_extra_magic_morning + park_temperature_average + park_ticket_season + park_temperature_high, 
  data = seven_dwarfs
)
```
...
```{r}
thirty <- wait_times |>
  filter(wait_minutes_posted_avg == 30)

sixty <- wait_times |>
  filter(wait_minutes_posted_avg == 60)
